    ...
    if request.method == 'GET':
        ...
        if form.validate():
            ...
            try:
                ...
                print("INSERT - Error en acceso a la base de datos: {}".format(e))
                conn.close()
            ...
            return render_template("newproduct.html", form=form,error_general=False)
...
def modifica_producto():
    if request.method == "GET":
        ...
        cur.execute(query,(id,)) #añadir la coma para que nos lo tome por tupla

        fila = cur.fetchone()
        conn.close()
        if fila:
            form = ModProductForm(data={"id":fila[0], "tipo_producto":fila[1], "precio_unitario":fila[2],"coste_unitario":fila[3]})
            return render_template("modproducto.html",form=form)
        else:
            return redirect(url_for("productos"))
    else:
